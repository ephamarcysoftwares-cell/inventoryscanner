...
import 'package:stock_and_inventory_software/phamacy/pharmacist_view%20report.dart';
...
import '../Scanner/UniversalScannerPage.dart';
...
// Helper function ya SnackBar
  ...
  Widget _buildResponsiveGrid(double width, Color cardColor, Color nmbBlue, Color textColor) {
    final Map<String, IconData> modules = {
      ...
      "DEBIT": Icons.payment,// Better for POS/Sales// Better for POS/Sales
      "REPORTS": Icons.assessment_rounded,   // Better for data/reports
  ...
  void _handleNavigation(String option) {
    ...
    if (option == "Add Normal Stock") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => SubAddMedicineScreen(user: user)));
    } else if (option == "View Normal Stock") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ViewMedicineScreen()));
    } else if (option == "Sales windows") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => SaleScreen(user: user)));
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => CartView(user: user)));
    } else if (option == "Manage users" || option == "MANAGE STORE") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => SuperViewUsersScreen(user: {},)));
    } else if (option == "add user") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => SubAdminRegisterScreenSfaff()));
    } else if (option == "Notify customer" || option == "Manage Customer") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => CustomerListScreen()));
    } // Inside _handleNavigation
    else if (option == "sales report") {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => PharmacistReportScreen(
            userRole: user['role']?.toString() ?? 'staff',
            userName: user['full_name']?.toString() ?? 'User',
          ),
        ),
      );
    } else if (option == "Normal product Stock report") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ViewMedicineStock()));
    } else if (option == "other product Stock report") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ProductLogsPage()));
    } else if (option == "view financial summary") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => FinancialSummaryScreen()));
    } else if (option == "Business Name") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => AddBusinessScreen()));
    } else if (option == "add company") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => AddCompanyScreen()));
    } else if (option == "add to store") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => AddMedicineStore(user: user)));
    } else if (option == "view my store") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => FinaViewStoreProducts()));
    } else if (option == "qr code manage") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => QrcodeGenerator()));
    } else if (option == "CCTV CONNECTION" || option == "VIEW SUSPECTED VIDEO") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => FFMPEGPlayerScreen()));
    } else if (option == "SET AUTOMATICAL SELL") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => CameraScreen(user: user)));
    } else if (option == "Manage Business Details") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ViewBusinessesScreen()));
    } else if (option == "Manage company") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ViewCompaniesScreen()));
    } else if (option == "view store migrated report") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => MigrationLogsScreen()));
    } else if (option == "view stock logs") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => MedicalLogsScreen()));
    }  else if (option == "Sales Analysis") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => BusinessSummaryScreen()));
    } else if (option == "Add Expenses") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => NormalUsageForm(userId: user['id'], addedBy: user['full_name'])));
    } else if (option == "import product") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ExcelReaderScreen()));
    } else if (option == "import company") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ExcelCompanyImport()));
    } else if (option == "view expenses") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ViewNormalUsageScreen()));
    } else if (option == "Salary payment") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => PaySalaryScreen()));
    } else if (option == "View salary history") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => SalaryTableScreen()));
    } else if (option == "deleted product history") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => DeletedMedicinesScreen()));
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => AllReceiptsScreen()));
    } else if (option == "Re -Stock") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => RestockCheckerScreenUser(user: user)));
    }  else if (option == "Permission") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => AssignPermissionsScreen(userId: user['id'], businessName: user['businessName'], userName: user['full_name'])));
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const ViewEventPage()),
      );
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => StoreTotalsScreen()));
    } else if (option == "stock analyse") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => FinishedMedicinesProfitScreen()));
    } else if (option == "View To Lend(MKOPO)") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ToLendReportScreen(userRole: user['role'] ?? '', userName: user['username'] ?? user['full_name'] ?? '')));
    } else if (option == "To Lend History") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => PaidLendLogsScreen(userRole: user['role'] ?? '', userName: user['username'] ?? user['full_name'] ?? '')));
    } else if (option == "Closing&opening Stock") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ClosingstockViewMedicineScreen()));
    } else if (option == "View Invoice") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ViewInvoiceScreen()));
    } else if (option == "Graphics") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => DesignEditorScreen(designType: '')));
    } else if (option == "Add other product") {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => OtherProduct(
            user: user,
            // Tunachukua staffId na full_name kutoka kwenye map ya user
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => AViewOtherProductScreen(user: user)));
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => ContractFormPage()));
...
// import 'pages/reverse_transaction_page.dart';
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => CustomerAnalyticsScreen()));
    } else if (option == "View Delivery Note") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ViewDeliveryNoteScreen()));
    } else if (option == "commonication setting") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => WhatsAppConfigScreen()));
    } else if (option == "Communication Message") {
      Navigator.push(context, MaterialPageRoute(builder: (_) => LiveWebhookMessages()));
    }else if (option == "View deleted product") {
      Navigator.push(context, MaterialPageRoute(builder: (_) =>  ViewEditHistory()));
    }else if (option == "Share what you have") {
      Navigator.push(context, MaterialPageRoute(builder: (_) =>  AdvancedChatScreen(currentUserName: '',)));
    }else if (option == "View my Diary") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const ViewDiaryPage()),
      );
    }else if (option == "Quick sales") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>UniversalTerminalPage (user: widget.user)),
      );
    }
  ...
  void _openServiceMenu(String module) {
    ...
    switch (module.toUpperCase()) {
      case "STOCK":
        opts = ["Add Normal Stock", "Add other product", "View other product", "View Normal Stock", "View Expired Product", "import product", ];
        break;
      case "SALES":
        opts = ["Sales windows", "view pending bill", "print receipt", "View To Lend(MKOPO)"];
        break;
      case "DEBIT":
        opts = ["View To Lend(MKOPO)","To Lend History"];
        break;
      case "REPORTS":
        opts = ["sales report", 'view expenses', ];
        break;
      case "USERS":
        opts = ["Manage users", "add user"];
        break;
      case "BUSINESS":
      ...
      case "BARCODE&QRCODE":
        ...
        break;
      case "EXPENSES":
        opts = ['Add Expenses', 'view expenses',];
        break;
      case "BRANCH":
        opts = ["Manage company", "add company","import company"];
        break;
      case "NOTIFICATION":
      ...
      case "AI/DETECTION":
        ...
        break;
      case "CONFIGURATION":
        opts = ["commonication setting", "Communication Message"];
        break;
      case "POS":
        opts = ["Quick sales"];
  ...
  Widget _buildNMBFloatingDock(Color nmbBlue) => Container(
    height: 70, // Tuliongeza urefu kidogo ili kitufe kikae vizuri
          ...
          onPressed: () => Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (context) => AdvancedChatScreen(
                      currentUserName: widget.user['username'] ?? 'Admin'
                  )
              )
          ),
        ),

        // 3. SCANNER BUTTON (Kitufe cha Katikati)
        Transform.translate(
          offset: const Offset(0, -5), // Kuinua kitufe kidogo juu ya bar
          child: GestureDetector(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => UniversalScannerPage(user: widget.user)),
              );
            },
            child: Container(
              padding: const EdgeInsets.all(12),
              decoration: const BoxDecoration(
                color: Colors.white, // Rangi nyeupe ili kionekane vizuri kwenye Blue
                shape: BoxShape.circle,
                boxShadow: [BoxShadow(color: Colors.black26, blurRadius: 5, spreadRadius: 1)],
              ),
              child: Icon(Icons.qr_code_scanner, color: nmbBlue, size: 30),
            ),
          ),
        ),

        // 4. CART ICON (Nimeongeza hii ili iwe rahisi kufika kwenye mauzo yanayofanyika sasa)
        IconButton(
        ...
        child: Column(
          children: [
            ...
            ListTile(
              ...
              title: Text("DASHBOARD", style: TextStyle(color: textColor)),
              onTap: () {
                // Inaonyesha ujumbe chini ya skrini (SnackBar)
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text(
                    ...
                    behavior: SnackBarBehavior.floating, // Inafanya ionekane kama inaelea
            ...
            ListTile(
              leading: const Icon(Icons.chat_bubble_outline, color: Colors.teal),
              title: Text("POS", style: TextStyle(color: textColor)),
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => UniversalTerminalPage (user: widget.user)));
              },
            ),
            const SizedBox(height: 20),
            const Spacer(),
