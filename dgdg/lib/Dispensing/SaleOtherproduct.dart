...
class _SaleOtherState extends State<SaleOther> {
  ...
  Future<void> loadOtherProducts() async {
    try {
      ...
      // 1. Pata Profile ya mtumiaji (Ongeza business_id hapa)
      final userProfile = await supabase
          .from('users')
          .select('role, business_name, sub_business_name, business_id') // ✅ Ongeza business_id
          .eq('id', user.id)
          .maybeSingle();

      if (!mounted || userProfile == null) return;

      // ✅ Hifadhi business_id kwenye variable
      final int? myBusinessId = userProfile['business_id'];
      final String myBusiness = userProfile['business_name'] ?? '';
      final String mySubBranch = userProfile['sub_business_name'] ?? '';
      final String myRole = userProfile['role']?.toString().toLowerCase() ?? 'staff';

      if (myBusinessId == null) {
        debugPrint("⚠️ [WARNING] Business ID haijapatikana kwa mtumiaji!");
        return;
      }

      // 2. Anza Query kwa kutumia business_id (Salama zaidi kuliko jina)
      var query = supabase.from('other_product').select().eq('business_id', myBusinessId);

      // 3. LOGIC YA MATAWI
      if (myRole == 'admin') {
        // Admin anaona bidhaa ambazo hazina tawi au ni Main Branch
        query = query.or('sub_business_name.is.null, sub_business_name.eq."", sub_business_name.eq."Main Branch"');
      } else {
        // Staff anaona bidhaa za tawi lake tu
        query = query.eq('sub_business_name', mySubBranch);
      }
      ...
      if (response is List) {
        ...
        setState(() {
          ...
          // ✅ WEKA KWENYE STATE ILI CART IWEZE KUITUMIA
          _businessId = myBusinessId;
          sub_business_name = mySubBranch;
        ...
        debugPrint("✅ Bidhaa ${allOtherProducts.length} zimepakiwa kwa Business ID: $myBusinessId");
      }
    } catch (e) {
    ...
    try {
      ...
      if (connectivityResult == ConnectivityResult.none) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No internet connection. Cloud cart requires sync.')),
        );
        return;
      }

      // 3. Fetch Real-time Stock from Supabase
      ...
      // 5. Prepare Cart Item (UPDATED WITH SUB_BUSINESS_NAME)
      final cartItem = {
        'user_id': userId,
        'business_id': _businessId, // ✅ IMEONGEZWA: Ili kutofautisha biashara
        ...
        // ✅ Tawi linatambulika (Null/Empty kama ni Main Store/Admin)
        'sub_business_name': sub_business_name,
      };

      // 6. Execute Supabase Updates
      // Update stock level (Kupunguza stock kwenye table husika)
      ...
      debugPrint("✅ SUPABASE SYNC SUCCESSFUL for branch: ${sub_business_name.isEmpty ? 'Main' : sub_business_name}");
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${product['name']} added to cart (${sub_business_name.isEmpty ? 'Main Store' : sub_business_name})'),
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(product['name'] ?? '', maxLines: 1, overflow: TextOverflow.ellipsis,
                                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: isExpired ? Colors.red : textCol)),

                            // ✅ Onyesha Tawi hapa
                            Text(itemBranch, style: const TextStyle(fontSize: 9, color: Colors.blue, fontWeight: FontWeight.bold, fontStyle: FontStyle.italic)),

                            const SizedBox(height: 2),
                            if (isExpired)
                              const Text("!!! EXPIRED !!!", style: TextStyle(color: Colors.red, fontSize: 8, fontWeight: FontWeight.bold)),

                            // ✅ Maelezo ya Stocked na Sold
                            Row(
                              children: [
                                Text("Stocked: $totalQuantity", style: const TextStyle(fontSize: 8, color: Colors.grey, fontWeight: FontWeight.bold)),
                                const SizedBox(width: 6),
                                Text("Sold: $soldQuantity", style: const TextStyle(fontSize: 8, color: Colors.orange, fontWeight: FontWeight.bold)),
                              ],
                            ),
                          ],
                        ),
