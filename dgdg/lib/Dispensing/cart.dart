...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> _fetchCartItems() async {
    ...
    try {
      ...
      if (isOnline) {
        // 1. Pata Profile ya aliyeingia ili kujua Role yake na Business ID yake
        final userProfile = await supabase
            .from('users')
        ...
        if (myRole == 'admin') {
          // ADMIN: Anaona vyote vya biashara hii (bila kujali user_id)
          query = query.eq('business_id', myBusinessId);
        } else {
          // STAFF: Anaona vyake tu alivyoscan yeye
        ...
        // Kwa offline, mara nyingi staff huona data za kwenye simu yake tu
                      ...
                      child: ListTile(
                        ...
                        title: Row(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                  color: branchName == "Main Branch" ? Colors.blue.withOpacity(0.1) : Colors.orange.withOpacity(0.1),
                                  borderRadius: BorderRadius.circular(8),
                                  border: Border.all(color: branchName == "Main Branch" ? Colors.blue : Colors.orange, width: 0.5)
                              ),
                              child: Text(
                                branchName,
                                style: TextStyle(
                                  fontSize: 10,
                                  fontWeight: FontWeight.bold,
                                  color: branchName == "Main Branch" ? Colors.blue : Colors.orange,
                                ),
                              ),
                            ),
                          ],
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Subtotal: TSH ${NumberFormat('#,##0.00').format(item['quantity'] * item['price'])}',
