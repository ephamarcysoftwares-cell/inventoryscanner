...
// Function to send the product restock notification email
  ...
  Future<void> getBusinessInfo() async {
    try {
      ...
      // Tunavuta business_name (Mama) na sub_business_name (Tawi)
      ...
      if (data != null) {
        String mainBiz = data['business_name'] ?? 'Unknown';
        String? subBiz = data['sub_business_name'];

        setState(() {
          // Kama sub_business_name ipo na si tupu, iweke hiyo kama jina la sasa la biashara
          // Vinginevyo tumia jina la biashara mama
          if (subBiz != null && subBiz.toString().trim().isNotEmpty) {
            businessName = subBiz.toString().toUpperCase();
          } else {
            businessName = mainBiz.toUpperCase();
          }
        });

        debugPrint('✅ Current UI Branch set to: $businessName');
      }
    } catch (e) {
...
// Helper method to refresh the UI variables
  ...
  Future<List<Map<String, dynamic>>> fetchMedicines({String searchQuery = ''}) async {
    ...
    try {
      debugPrint('🚀 Starting fetchMedicines...');

      // 1. Connectivity Check (Modern Way)
      var connectivityResults = await (Connectivity().checkConnectivity());

      // Angalia kama kweli hamna internet kabisa
      bool hasNoNet = connectivityResults.contains(ConnectivityResult.none) || connectivityResults.isEmpty;

      if (hasNoNet) {
        debugPrint('⚠️ Connectivity plugin says No Internet. Attempting anyway...');
        // Haturudishi [] hapa mara moja ili kuruhusu Supabase ijaribu yenyewe
      }
      ...
      // 2. Vuta Profile ya mtumiaji (Pia inatusaidia kuona kama server inafikika)
      ...
      final myBizId = userDoc['business_id'];
      final mySubBranch = userDoc['sub_business_name'];
      final String myRole = (userDoc['role'] ?? '').toString().trim().toLowerCase();

      // 3. Anza Query ya Medicines
      var supabaseQuery = supabase
          .from('medicines')
          .select()
          .eq('business_id', myBizId);

      // 4. ROLE LOGIC & BRANCH FILTERING
      // Ikiwa mtumiaji si Storekeeper, aone bidhaa za tawi lake tu
      if (myRole != 'storekeeper') {
        if (mySubBranch != null && mySubBranch.toString().trim().isNotEmpty) {
          supabaseQuery = supabaseQuery.eq('sub_business_name', mySubBranch);
        } else {
          // Ikiwa hana sub_branch, anachukua bidhaa za Main Branch au ambazo hazina tawi
          supabaseQuery = supabaseQuery.or('sub_business_name.is.null,sub_business_name.eq."",sub_business_name.eq."Main Branch"');
        }
      }

      // 5. Search Logic (Name, Company, or Batch Number)
      if (query.isNotEmpty) {
        supabaseQuery = supabaseQuery.or('name.ilike.%$query%,company.ilike.%$query%,batch_number.ilike.%$query%');
      }
      ...
      if (response is List) {
        ...
        debugPrint('✅ Fetch Successful: Found ${rawResults.length} items for ${mySubBranch ?? "Main"}');

        // 🔥 AUTO GENERATE MISSING QR CODES 🔥
        if (rawResults.isNotEmpty) {
          _autoGenerateMissingQRCodes(rawResults);
        }
      }
      ...
      // Hapa unaweza kuonyesha Toast kwa mtumiaji ikiwa ni Error ya Connection
      return [];
    }
  }

  /// Hii function inapitia orodha na ku-update database kiotomatiki
  /// kwa kila bidhaa ambayo haina QR Code URL.
  Future<void> _autoGenerateMissingQRCodes(List<Map<String, dynamic>> list) async {
    final supabase = Supabase.instance.client;

    // Tafuta bidhaa ambazo column ya qr_code_url ni null au tupu
    final missingQR = list.where((m) =>
    m['qr_code_url'] == null || m['qr_code_url'].toString().trim().isEmpty
    ).toList();

    if (missingQR.isEmpty) {
      debugPrint('✨ All products already have QR codes.');
      return;
    }

    debugPrint('🛠️ Auto-generating ${missingQR.length} missing QR codes background...');

    for (var med in missingQR) {
      try {
        // Tunatumia item_code (Standard) au ID kama data ya QR
        final String qrData = (med['item_code'] != null && med['item_code'].toString().isNotEmpty)
            ? med['item_code']
            : "PROD-${med['id']}";

        // Update Column ya qr_code_url kwenye Database
        await supabase
            .from('medicines')
            .update({'qr_code_url': qrData})
            .eq('id', med['id']);

        debugPrint("✅ Fixed QR for: ${med['name']}");
      } catch (e) {
        debugPrint("❌ Error updating QR for ${med['name']}: $e");
      }
    }

    // Tunaita loadMedicines() mara moja tu mwishoni kurefresh UI ionyeshe Green Icons
    if (mounted) {
      setState(() {
        medicines = fetchMedicines(searchQuery: searchController.text);
      });
    }
  }
...
// Helper to handle mixed types from SQLite (Real) and Supabase (Int/Numeric)
  ...
  // ================== Build UI ==================
  void _showQRCodeDialog(Map<String, dynamic> med) {
    String qrData = med['qr_code_url'] ?? med['item_code'] ?? "NO-DATA";
    String prodName = med['name'] ?? "PRODUCT";

    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Text(prodName, textAlign: TextAlign.center, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        content: SingleChildScrollView( // Ongeza hii kuzuia overflow na size errors
          child: Column(
            ...
            children: [
              Screenshot(
                ...
                child: Container(
                  width: 250, // WEKA SIZE HAPA ili kuzuia RenderBox error
                  padding: const EdgeInsets.all(15),
                  color: Colors.white,
                  child: Column(
                    children: [
                      Text(prodName, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 12)),
                      const SizedBox(height: 10),
                      // Hakikisha QrImageView ina size
                      SizedBox(
                        height: 180,
                        width: 180,
                        child: QrImageView(data: qrData, size: 180, version: QrVersions.auto),
                      ),
                      const SizedBox(height: 5),
                      Text(businessName, style: const TextStyle(color: Colors.black54, fontSize: 9)),
                    ],
                  ),
                ),
              ),
              const SizedBox(height: 15),
              const Text("MAELEKEZO:", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue, fontSize: 12)),
              const Text("Pakua picha hii kisha iprint. Bandika sticker hii kwenye bidhaa ili uweze kuiskeni Bidhaa bila kuingia kwenye mfumo wakati wa mauzo.", textAlign: TextAlign.center, style: TextStyle(fontSize: 10)),
              const SizedBox(height: 15),
              ElevatedButton.icon(
                onPressed: () async {
                  // Capture screenshot
                  ...
                  if (image != null) {
                    await FileSaver.instance.saveFile(name: "QR_${prodName}", bytes: image, ext: "png", mimeType: MimeType.png);
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ QR Imepakuliwa!")));
                  }
                },
      ...
      appBar: AppBar(
        title: Column(
          children: [
            ...
            // Inaonyesha 'All Branches' kama ni Storekeeper
            Text(
              userRole == 'storekeeper' ? "MODE: ALL BRANCHES (STOREKEEPER)" : businessName,
        ...
        centerTitle: true,
        elevation: 0,
        // Inaongeza kitufe cha Refresh ili kurahisisha kazi ya Storekeeper
        actions: [
          IconButton(
              onPressed: loadMedicines,
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Row( // Ondoa 'const' hapa kwa sababu tunatumia variable ya businessName
                children: [
                  // 1. Jina la Bidhaa
                  const Expanded(
                      flex: 3,
                      child: Text('Name', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10))
                  ),

                  // 2. Jina la Tawi (Inatumia businessName uliyobadilisha juu)
                  // Expanded(
                  //     flex: 3,
                  //     child: Text('Branch ($businessName)',
                  ...
                  const Expanded(
                      flex: 1,
                      child: Text('Rem', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10), textAlign: TextAlign.center)
                  ),

                  // 4. Bei
                  const Expanded(
                      flex: 2,
                      child: Text('Price', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10), textAlign: TextAlign.center)
                  ),

                  // 5. Expiry
                  const Expanded(
                      flex: 2,
                      child: Text('Expiry', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10), textAlign: TextAlign.center)
                  ),
                      ...
                      return Container(
                        ...
                        child: ListTile(
                            ...
                            children: [
                              ...
                              Expanded(
                                ...
                                child: Text(
                                  ...
                                  softWrap: true, // Inaruhusu maandishi kushuka mstari
                                  overflow: TextOverflow.visible, // Inahakikisha herufi zote zinaonekana bila kukatwa
                                  // maxLines: 2, // Unaweza kuweka hii ukihitaji isizidi mistari miwili
                                ),
                              ),
                              ...
                              //   flex: 3,
                              //   child: Text(
                              //     (med['sub_business_name']?.toString().isEmpty ?? true)
                              //         ? "MAIN"
                              //         : med['sub_business_name'].toString().toUpperCase(),
                              //     style: TextStyle(
                              //         fontSize: 10,
                              //         color: isDark ? lightViolet : deepPurple,
                              ...
                              Expanded(
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    fontSize: 11,
                                    fontWeight: FontWeight.bold,
                                    color: (med['remaining_quantity'] ?? 0) < 5 ? Colors.red : Colors.green,
                                  ),
                                ),
                              ),
                          ...
                          trailing: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              // --- KITUFE CHA QR CODE (NEW) ---
                              IconButton(
                                icon: const Icon(Icons.qr_code, color: Colors.indigo, size: 20),
                                onPressed: () {
                                  _showQRCodeDialog(med);
                                },
                              ),
                              // ADD STOCK
                              IconButton(
