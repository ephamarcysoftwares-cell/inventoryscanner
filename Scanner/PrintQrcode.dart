...
class _ViewAllQRCodesPageState extends State<ViewAllQRCodesPage> {
  ...
  Future<void> _printSelectedQRs() async {
    ...
    final rawItems = _filteredItems.where((i) => i['code'].isNotEmpty).toList();
    if (rawItems.isEmpty) return;

    // 1. PANGA DATA KWA ALFABETI (A-Z)
    rawItems.sort((a, b) => a['name'].toString().toUpperCase().compareTo(b['name'].toString().toUpperCase()));

    // 2. GROUP DATA KWA HERUFI YA KWANZA
    Map<String, List<Map<String, dynamic>>> groupedItems = {};
    for (var item in rawItems) {
      String firstLetter = item['name'].toString().trim().substring(0, 1).toUpperCase();
    ...
    final sortedLetters = groupedItems.keys.toList()..sort();

    for (var letter in sortedLetters) {
      // --- STEP A: UKURASA WA UTAMBULISHO WA HERUFI (INTRO PAGE) ---
      pdf.addPage(pw.Page(
        ...
        build: (pw.Context context) => pw.Container(
          ...
          decoration: pw.BoxDecoration(
            border: pw.Border.all(color: PdfColors.teal700, width: 5),
          ),
          child: pw.Stack(
            alignment: pw.Alignment.center,
            children: [
              // Herufi Kubwa Iliyopinda (Background)
              pw.Transform.rotate(
                angle: -0.5,
                child: pw.Text(
                  letter,
                  style: pw.TextStyle(
                    ...
                    color: PdfColor.fromHex('#15008080'), // Opacity hafifu sana
                  ),
                ),
              ),
              ...
              pw.Column(
                ...
                children: [
                  pw.Text("SASA UNAANZA",
                      style: pw.TextStyle(fontSize: 25, color: PdfColors.grey700, letterSpacing: 2)),
                  pw.SizedBox(height: 15),
                  pw.Text("HERUFI ($letter)",
                      style: pw.TextStyle(fontSize: 80, fontWeight: pw.FontWeight.bold, color: PdfColors.teal900)),
                  ...
                  pw.Container(
                    ...
                    child: pw.Text(
                      "AHSANTE KWA KUCHAGUA STOCK AND INVENTORY SOFTWARE",
      ...
      // --- STEP B: KURASA ZA DATA (ORODHA YA QR CODES) ---
      final letterItems = groupedItems[letter]!;
      int itemsPerPage = _isVerticalLayout ? 7 : 32;

      for (var i = 0; i < letterItems.length; i += itemsPerPage) {
        final chunk = letterItems.sublist(
            i, i + itemsPerPage > letterItems.length ? letterItems.length : i + itemsPerPage);

        pdf.addPage(pw.Page(
          pageFormat: PdfPageFormat.a4,
          build: (pw.Context context) => pw.Column(
            children: [
              // Header ya Mwendelezo
              pw.Container(
                width: double.infinity,
                ...
                child: pw.Row(
                  ...
                  children: [
                    pw.Text(title, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.white)),
                    pw.Text("MWENDELEZO: ($letter)", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.white)),
                  ],
                ),
              ),
              pw.SizedBox(height: 20),

              // Orodha ya QR (Vertical au Grid)
              if (_isVerticalLayout)
                  ...
                  children: chunk.map((item) => pw.Container(
                    ...
                    child: pw.Row(children: [
                      ...
                      pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                        pw.Text(item['name'].toUpperCase(), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
                        pw.Text("CODE: ${item['code']}", style: pw.TextStyle(fontSize: 10, color: PdfColors.teal900, fontWeight: pw.FontWeight.bold)),
                      ]),
                    ]),
                  )).toList(),
                )
                ...
                pw.Wrap(
                  ...
                  children: chunk.map((item) => pw.Container(
                    width: (PdfPageFormat.a4.width - 100) / 4,
                    padding: const pw.EdgeInsets.all(6),
                    decoration: pw.BoxDecoration(border: pw.Border.all(color: PdfColors.grey300, width: 0.5)),
                    child: pw.Column(children: [
                      pw.BarcodeWidget(barcode: pw.Barcode.qrCode(), data: item['code'], width: 45, height: 45),
                      pw.SizedBox(height: 4),
                      pw.Text(item['name'], style: const pw.TextStyle(fontSize: 7), maxLines: 1),
                      pw.Text(item['code'], style: pw.TextStyle(fontSize: 6, fontWeight: pw.FontWeight.bold)),
                    ]),
                  )).toList(),
                ),
              ...
              // Branding Footer kwenye kila ukurasa
              pw.Divider(color: PdfColors.teal700, thickness: 1),
              pw.Row(
                mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                children: [
                  pw.Text("AHSANTE KWA KUCHAGUA STOCK AND INVENTORY SOFTWARE",
                      style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: PdfColors.teal700)),
    ...
    await Printing.layoutPdf(onLayout: (format) async => pdf.save(), name: 'QR_Alphabetical_Report');
  }
}