...
class SubAdminViewUsersScreen extends StatefulWidget {
  const SubAdminViewUsersScreen({super.key});

  @override
  _SubAdminViewUsersScreenState createState() => _SubAdminViewUsersScreenState();
}

class _SubAdminViewUsersScreenState extends State<SubAdminViewUsersScreen> {
  ...
  Future<void> fetchUsers() async {
    ...
    try {
      ...
      // 1. Pata profile ya aliyelogin sasa hivi (Pata business_id yake)
      final myProfile = await supabase
          .from('users')
          .select('email, business_id, business_name')
          .eq('id', currentUser.id)
          .single();

      final String myEmail = myProfile['email'] ?? "";
      final dynamic myBizId = myProfile['business_id']; // Hii ndio nanga yetu
      final String myBizName = myProfile['business_name'] ?? "";

      setState(() {
        _currentAdminBusiness = myBizName;
      });

      // 2. Anza Query ya users
      var query = supabase.from('users').select();

      if (myEmail == superAdminEmail) {
        // KENNEDY: Anaona kila mtu
        debugPrint("Super Admin identified: Loading all users globally.");
      } else {
        // ADMIN MWINGINE: Lazima aone wa Tawi lake pekee (kwa business_id)
        debugPrint("Standard Admin identified: Filtering by Business ID: $myBizId");

        if (myBizId != null) {
          // Tunatumia .eq('business_id', ...) badala ya jina
          query = query.eq('business_id', myBizId);
        } else {
          // Kama hana business_id, basi aone wa jina lake (Back-up logic)
          query = query.eq('business_name', myBizName);
        }
      }
