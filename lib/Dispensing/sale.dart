...
// --- CORRECTED DISCOUNT DIALOG WITH WORKING CHECKBOX ---
  ...
  Future<void> _addToCart(Map<String, dynamic> medicine, int quantity, {double? adminDiscount}) async {
    ...
    try {
      ...
      // 1. Pata Profile - Hakikisha unachukua business_id hapa!
      final userProfile = await supabase
          .from('users')
          .select('business_name, sub_business_name, business_id') // ✅ Ongeza business_id hapa
          .eq('id', userId)
          .maybeSingle();

      // 2. Verify Stock, Cloud Price, na Business ID ya bidhaa
      final cloudData = await supabase
          .from('medicines')
          .select('remaining_quantity, price, discount, business_id') // ✅ Chukua business_id ya bidhaa
          .eq('id', medicine['id'])
          .maybeSingle();
      ...
      // 3. Logic ya kupata Business ID ya uhakika
      // Tunapendelea ile ya kwenye bidhaa, ikikosekana tunachukua ya mtumiaji
      final dynamic finalBusinessId = cloudData['business_id'] ?? userProfile?['business_id'];

      if (finalBusinessId == null) {
        throw Exception("Business ID missing. Please check database.");
      }
      ...
      final cartItem = {
        ...
        'business_id': finalBusinessId, // ✅ Sasa imehakikishwa (Sio null tena)
        'business_name': userProfile?['business_name'] ?? business_name,
        'sub_business_name': userProfile?['sub_business_name'] ?? '', // ✅ Kinga ya null
      };

      // 6. UPDATE STOCK & INSERT TO CART (Atomic operations)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        // 2. Hii inazuia Flutter isiongeze back button yake ya asili (nyeusi)
        automaticallyImplyLeading: false,

        title: const Text("DISPENSING WINDOW",
            style: TextStyle(color: Colors.white, fontWeight: FontWeight.w300, fontSize: 18, letterSpacing: 1.2)),
        centerTitle: true,
        elevation: 0,
