...
""";
  ...
  Future<List<Map<String, dynamic>>> fetchLendData() async {
    try {
      ...
      Map<String, Map<String, dynamic>> groupedReceipts = {};

      for (var row in rawData) {
        String receipt = row['receipt_number'] ?? 'N/A';
        double price = (double.tryParse(row['total_price'].toString()) ?? 0.0);

        if (!groupedReceipts.containsKey(receipt)) {
          groupedReceipts[receipt] = Map<String, dynamic>.from(row);
          groupedReceipts[receipt]!['total_price'] = price;

          // MUHIMU: Hapa ndipo tunatengeneza hiyo list inayotafutwa na UI
          groupedReceipts[receipt]!['detailed_products'] = [{
            'name': row['medicine_name'] ?? 'Unknown',
            'qty': row['total_quantity'] ?? 0,
            'price': price,
          }];
        } else {
          groupedReceipts[receipt]!['total_price'] += price;
          // Ongeza bidhaa kwenye list iliyopo
          groupedReceipts[receipt]!['detailed_products'].add({
            'name': row['medicine_name'] ?? 'Unknown',
            'qty': row['total_quantity'] ?? 0,
            'price': price,
          });
      ...
      return groupedReceipts.values.toList();
    } catch (e) {
      debugPrint("❌ Error grouping data: $e");
      return [];
    }
  }
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              title: Text('MALIPO YA SEHEMU: #$receipt',
              ...
              content: Column(
                ...
                children: [
                  ...
                  TextField(controller: amountController,
                      decoration: const InputDecoration(hintText: 'Kiasi kinacholipwa (TSH)'),
                      keyboardType: TextInputType.number),
                  if (isSubmitting) const Padding(padding: EdgeInsets.only(top: 10),
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: isSubmitting ? null : () async {
                    double amountPaid = double.tryParse(amountController.text.trim()) ?? 0.0;

                    // Tunapata jumla ya deni lote la risiti hii kwanza
                    double totalReceiptDebt = double.tryParse(lendRecord['total_price'].toString()) ?? 0.0;

                    if (amountPaid <= 0 || amountPaid > totalReceiptDebt) {
                      ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Kiasi si halali au kinazidi deni')));
                      return;
                    }
                    ...
                    try {
                      ...
                      // 1. VUTA BIDHAA ZOTE ZA RISITI HII KUTOKA DB
                      final response = await supabase
                          .from('To_lend')
                          .select('id, total_price, medicine_name, total_quantity, sub_business_name')
                          .eq('receipt_number', receipt)
                          .eq('business_id', bId)
                          .order('id', ascending: true);

                      List<dynamic> items = response as List;
                      double remainingToDistribute = amountPaid;

                      // 2. GAWA MALIPO KWA KILA ROW KWA KUTUMIA ID
                      for (var item in items) {
                        if (remainingToDistribute <= 0) break;

                        int itemId = item['id'];
                        double itemDebt = double.tryParse(item['total_price'].toString()) ?? 0.0;

                        if (remainingToDistribute >= itemDebt) {
                          // Deni la bidhaa hii limeisha
                          remainingToDistribute -= itemDebt;
                          await supabase.from('To_lend').delete().eq('id', itemId);
                        } else {
                          // Punguza deni kidogo na baki na balance
                          double newBalance = itemDebt - remainingToDistribute;
                          remainingToDistribute = 0;
                          await supabase.from('To_lend').update({
                            'total_price': newBalance,
                            'customer_phone': phoneController.text,
                          }).eq('id', itemId); // <--- HAPA NDIPO TUNATUMIA ID
                        }
                      }
                      ...
                      Map<String, dynamic> logData = {
                        'customer_name': cName,
                        'customer_phone': phoneController.text,
                        'customer_email': lendRecord['customer_email'],
                        'receipt_number': receipt,
                        'total_price': amountPaid,
                        'medicine_name': lendRecord['medicine_name'] ?? 'Deni la Dawa',
                        'total_quantity': lendRecord['total_quantity'] ?? 1,
                        'payment_method': 'Partial Cash',
                        'confirmed_time': DateTime.now().toIso8601String(),
                        'paid_time': DateTime.now().toIso8601String(),
                        'user_id': user.id,
                        'business_id': bId,
                        'business_name': business_name,
                        'sub_business_name': lendRecord['sub_business_name'],
                        'full_name': staff,
                        'confirmed_by': staff,
                        'synced': 1,
                        'source': 'DEBT PAYMENT'
                      ...
                      await supabase.from('To_lent_payedlogs').insert(logData);
                      await supabase.from('sales').insert(logData);

                      // 4. NOTIFICATIONS
                      _sendPaymentNotifications(
                        customerName: cName,
                        customerPhone: phoneController.text,
                        amountPaid: NumberFormat('#,##0').format(amountPaid),
                        balanceRemaining: NumberFormat('#,##0').format(totalReceiptDebt - amountPaid),
                        receipt: receipt,
                      );
  ...
  Future<void> _markAsPaid(Map<String, dynamic> lendRecord) async {
    ...
    final double amount =
        double.tryParse(lendRecord['total_price'].toString()) ?? 0.0;
    final String cPhone = lendRecord['customer_phone'] ?? '';
    final dynamic bId = lendRecord['business_id'] ?? currentBusinessId;

    showDialog(
      context: context,
      barrierDismissible: false, // Inazuia kufunga dialog kwa bahati mbaya wakati inaprocess
      builder: (dialogContext) {
        bool isLoading = false; // Hii itatunza hali ya mzunguko
          ...
          builder: (context, setState) {
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text(
                    'Uthibitishe kupokea TSH ${NumberFormat('#,##0').format(amount)} '
                        'kutoka kwa $cName kwa risiti #$receipt?',
                  ),
                  if (isLoading) ...[
                    const SizedBox(height: 20),
                    const CircularProgressIndicator(color: Colors.green),
                    const SizedBox(height: 10),
                    const Text("Inachakata malipo...", style: TextStyle(fontSize: 12, color: Colors.grey)),
                  ],
                ],
              ),
              actions: [
                // Ficha Cancel kama tayari inaprocess
                if (!isLoading)
                  TextButton(
                ...
                TextButton(
                  // Kama inaload, kitufe kinakuwa "disabled"
                  onPressed: isLoading
                      ? null
                      : () async {
                    setState(() => isLoading = true); // Anzisha mzunguko
                    ...
                    try {
                      ...
                      Map<String, dynamic> paymentData = {
                        ...
                        'receipt_number': receipt,
                        'total_price': amount,
                        'medicine_name': lendRecord['medicine_name'] ?? 'Deni Kamili',
                        'total_quantity': lendRecord['total_quantity'] ?? 1,
                        'payment_method': 'Full Cash',
                        'confirmed_time': DateTime.now().toIso8601String(),
                        'paid_time': DateTime.now().toIso8601String(),
                        'confirmed_by': staff,
                        'full_name': staff,
                        'user_id': user.id,
                        'business_id': bId,
                        'business_name': business_name, // Kutoka kwenye kumbu kumbu zako za Arusha Online Store
                        'sub_business_name': lendRecord['sub_business_name'],
                        'synced': 1,
                        'source': 'DEBT PAYMENT'
                      };
                      ...
                      // Funga dialog sasa baada ya kila kitu kuisha
                      Navigator.pop(dialogContext);

                      scaffoldMessenger.showSnackBar(
                        const SnackBar(
                          content: Text('Malipo yamekamilika na deni limefutwa!'),
                          backgroundColor: Colors.green,
                        ),
                      );
                    } catch (e) {
                      // Zima mzunguko kama kuna error ili mtumiaji aone kosa au ajaribu tena
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        toolbarHeight: 90,
        // 1. Kulazimisha icon ya back iwe nyeupe
        leading: IconButton(
          icon: const Icon(Icons.arrow_back, color: Colors.white),
          onPressed: () => Navigator.of(context).pop(),
        ),
        // 2. Kuzuia icon ya asili ya mfumo (ambayo mara nyingi ni nyeusi)
            ...
            child: isLoading
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemCount: snapshot.data!.length,
                  itemBuilder: (context, index) {
                    // Hii 'sale' sasa inawakilisha kundi la risiti (Grouped Receipt)
                    final sale = snapshot.data![index];
                    String receipt = sale['receipt_number'] ?? '';

                    // Tunatayarisha controllers za mawasiliano
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        color: cardColor,
                        borderRadius: BorderRadius.circular(20),
                        boxShadow: [BoxShadow(color: Colors.black.withOpacity(_isDarkMode ? 0.3 : 0.03), blurRadius: 10)],
                      ),
                      child: Padding(
                        padding: const EdgeInsets.all(16),
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                Container(
                                  ...
                                  child: Text("#$receipt", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.deepPurple, fontSize: 12)),
                                ),
                            ...
                            Divider(height: 24, color: _isDarkMode ? Colors.white10 : Colors.grey[200]),

                            // Contact Inputs
                            _buildInCardField(phoneControllers[receipt]!, "Phone", Icons.phone, TextInputType.phone, textCol, _isDarkMode),
                            const SizedBox(height: 8),
                            _buildInCardField(emailControllers[receipt]!, "Email", Icons.email, TextInputType.emailAddress, textCol, _isDarkMode),
                            ...
                            Container(
                              ...
                              child: Column(
                                children: [
                                  _buildFinanceRow("Original Total", _fetchOriginalTotal(receipt), Colors.teal, subTextCol),
                                  _buildFinanceRow("Total Paid", _fetchTotalPaid(receipt), Colors.orange, subTextCol),
                                  Divider(height: 16, color: _isDarkMode ? Colors.white10 : Colors.grey[300]),
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text("BALANCE DUE", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: textCol)),
                                      Text(
                                        "TSH ${NumberFormat('#,##0').format(double.tryParse(sale['total_price'].toString()) ?? 0.0)}",
                                        style: const TextStyle(fontWeight: FontWeight.w900, color: Colors.redAccent, fontSize: 18),
                                      ),
                                    ],
                                  ),
                                ],
                            ...
                            // ✅ MCHANGANUO WA BIDHAA (QUANTITY & PRICE PER ITEM)
                            Text("ITEM BREAKDOWN", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: subTextCol, letterSpacing: 1)),
                            const SizedBox(height: 8),
                            Container(
                              width: double.infinity,
                              padding: const EdgeInsets.all(12),
                              ...
                              child: Column(
                                children: [
                                  // Hapa tunapita kwenye kila bidhaa iliyopo kwenye list ya 'detailed_products'
                                  ...(sale['detailed_products'] as List? ?? []).map((item) {
                                    return Padding(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Expanded(
                                            flex: 2,
                                            child: Text(
                                              "TSH ${NumberFormat('#,##0').format(item['price'])}",
                                              textAlign: TextAlign.right,
                                              style: TextStyle(fontSize: 13, color: textCol, fontWeight: FontWeight.bold),
                                            ),
                                          ),
                                        ],
                                      ),
                            ...
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 8),
                                _buildActionBtn(Icons.done_all, "Clear All", Colors.green, () => _markAsPaid(sale)),
                              ],
                            ),
                          ],
                        ),
