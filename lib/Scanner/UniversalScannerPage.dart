...
// --------------------------------------------------------------------------
  Future<void> _executeRestock(Map<String, dynamic> item, double add, String source) async {
    ...
    // LOG: Kuanza mchakato (Console Debug)
    debugPrint("🚀 RESTOCK START: Item=${item['name'] ?? item['medicine_name']}, Add=$add, Source=$source");
    ...
    try {
      ...
      debugPrint("📂 DATABASE ROUTE: MainTable=$mainTable, LogTable=$logTable");

      // -------------------------------------------------------------------------
      // HATUA YA 3: UPDATE STOCK KWENYE MEZA KUU
      // Tunatumia .toInt() hapa ili kuzuia kosa la "1.0" kwenye INTEGER columns
      // -------------------------------------------------------------------------
      await _supabase
          .from(mainTable)
          .update({
      ...
      debugPrint("✅ STOCK UPDATED ON $mainTable");

      // -------------------------------------------------------------------------
      // HATUA YA 4: REKODI TUKIO KWENYE LOGS (Kwa ajili ya Details View)
      // -------------------------------------------------------------------------
      Map<String, dynamic> logData = {
        nameColumn: item['name'] ?? item['medicine_name'] ?? item['service_name'],
        'total_quantity': add.toInt(), // Geuza kuwa integer safi
        'action': 'Restock: Aliongeza bidhaa',
        'added_by': widget.user['username'] ?? widget.user['full_name'] ?? 'Staff',
        'business_id': widget.user['business_id'],
        'business_name': widget.user['business_name'],
        'date_added': DateTime.now().toIso8601String(),
      };

      // Maalum kwa medical_logs: column ya remaining_quantity ni numeric (inakubali decimal)
      // Lakini kwa meza zingine ni integer, hivyo tunatumia .toInt()
      if (source == 'NORMAL PRODUCT') {
        logData['remaining_quantity'] = newTotalQty;
      } else {
        logData['remaining_quantity'] = newTotalQty.toInt();
      }

      debugPrint("✍️ WRITING LOG: $logData");

      await _supabase.from(logTable).insert(logData);

      debugPrint("🎊 RESTOCK PROCESS COMPLETED SUCCESSFULLY");

      // 5. MREJESHO KWA MTUMIAJI
      _showNotification("Stock ya $source Imesasishwa!", Colors.green);
      _speak("Tayari, mzigo mpya wa ${item['name'] ?? item['medicine_name'] ?? ''} umeingizwa");

    } catch (e, stackTrace) {
      debugPrint("❌ RESTOCK FATAL ERROR: $e");
      debugPrint("📜 STACKTRACE: $stackTrace");
      _showNotification("Imeshindwa ku-update: $e", Colors.red);
    } finally {
