...
class _SalesReportScreenState extends State<SalesReportScreen> {
  ...
  void _applyFilters() async {
    ...
    try {
      // 1. Vuta data kutoka Supabase RPC
      final List<Map<String, dynamic>> rpcRows = await _fetchSalesData();

      debugPrint("--- 🔍 DEBUG START ---");
      debugPrint("Total Rows: ${rpcRows.length}");

      double tempSales = 0.0;
      double tempPaidDebt = 0.0;
      double tempNewDebt = 0.0;
      double tempServices = 0.0;

      // List ya kukusanya data za grafu (Cash pekee au na madeni yaliyolipwa)
      List<double> tempTrend = [];
      Map<String, double> tempBySource = {};

      for (var row in rpcRows) {
        // ✅ Usalama wa Business ID
        if (_realRoleFromDB != 'admin') {
          if (row['business_id']?.toString() != currentBusinessId?.toString()) {
            continue;
          }
        }

        // ✅ Kanuni ya sub_business_name (kama ni string 'NULL' iwe null halisi)
        String? subBiz = row['sub_business_name']?.toString();
        if (subBiz == 'NULL' || subBiz == '') subBiz = null;
        ...
        // --- 1. SHUGHULIKIA HUDUMA (Services) ---
        if (table == 'services') {
          tempServices += price;
          tempBySource["Other Products ($source)"] = (tempBySource["Other Products ($source)"] ?? 0.0) + price;
          if (price > 0) tempTrend.add(price);
          continue;
        }

        // --- 2. SHUGHULIKIA MALIPO YA MADENI (To_lent_payedlogs) ---
        if (table == 'logs' || table == 'to_lent_payedlogs') {
          tempPaidDebt += price;
          tempBySource["Debt Paid ($source)"] = (tempBySource["Debt Paid ($source)"] ?? 0.0) + price;
          // Malipo ya deni ni cash iliyoingia leo, tunaweka kwenye graph
          if (price > 0) tempTrend.add(price);
          continue;
        }

        // --- 3. SHUGHULIKIA MADENI MAPYA (Lend / To_Lend) ---
        // REKEBISHO: Tumeondoa 'processedDebtReceipts' ili bidhaa zote za risiti moja zijumlishwe
        if (table == 'lend' || table == 'to_lend') {
          tempNewDebt += price;
          tempBySource["New Debt ($source)"] = (tempBySource["New Debt ($source)"] ?? 0.0) + price;
          // Madeni mapya mara nyingi hayawekwi kwenye graph ya 'Cash Flow' mpaka yalipwe
          continue;
        }

        // --- 4. SHUGHULIKIA MAUZO YA KAWAIDA (Sales) ---
        if (table == 'sales') {
          if (method.contains('FULL') || method.contains('CASH') || method.contains('PARTIAL')) {
            tempSales += price;
            tempBySource["Cash Sales ($source)"] = (tempBySource["Cash Sales ($source)"] ?? 0.0) + price;
            if (price > 0) tempTrend.add(price);
          }
        }
      }
                    ...
                    child: isLoading
                      ...
                      builder: (context, snapshot) {
                        ...
                        final filteredList = snapshot.data!.where((item) {
                          ...
                          if (selectedCategory == "NORMAL PRODUCT") return src.contains("NORMAL") || src.contains("MEDICINE");
                                ...
                                child: ConstrainedBox(
                                  ...
                                  child: DataTable(
                                    ...
                                    columns: [
                                      ...
                                      const DataColumn(label: Text('Staff', style: TextStyle(color: Colors.white, fontSize: 11))),
                                    ...
                                    rows: filteredList.map((sale) {
                                      ...
                                      return DataRow(
                                        ...
                                        cells: [
                                          ...
                                          // Staff Cell
                                          DataCell(Text(staffName, style: const TextStyle(fontSize: 9, color: Colors.blueGrey))),
