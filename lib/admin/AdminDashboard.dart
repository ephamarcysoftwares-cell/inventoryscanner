...
import '../Dispensing/ControlNumberScreen.dart';
...
import '../OTHER SERVICES/ReportServices.dart';
import '../OTHER SERVICES/SalesOtherCervices.dart';
import '../OTHER SERVICES/ServiceLogsPage.dart';
import '../OTHER SERVICES/ViewAllservices.dart';
...
import '../Reviser_trasaction/Sales_Reserve.dart';
import '../STORE/edit_store.dart';
import '../STORE/view_bussines details.dart';
import '../STORE/view_company.dart';
import '../SUBADMIN/ACCOUNTANT/restock_checker.dart';
import '../SUBADMIN/ACCOUNTANT/view_other_product.dart';
import '../SUBADMIN/view_users.dart';
import '../SalesReportScreen.dart';
import '../Scanner/PrintQrcode.dart';
import '../Scanner/ScannerScreen.dart';
...
import '../login.dart';
import '../phamacy/STORE/add_store_medicine.dart';
...
import 'Other services.dart';
import 'REPORT/ViewMedicineStockDeleted.dart';
import 'REPORT/view_history_logs.dart';
...
// Helper function ya SnackBar
  ...
  Widget _buildResponsiveGrid(double width, Color cardColor, Color nmbBlue, Color textColor) {
    final Map<String, IconData> modules = {
      ...
      "QRCODE PRODUCT":Icons.qr_code_scanner_rounded,
      ...
      "REPORT SERVICES":Icons.report,
      "SALES SERVICES":Icons.sell,
  ...
  void _handleNavigation(String option) {
    ...
    if (option == "Add Normal Stock") {
      Navigator.push(context,
          MaterialPageRoute(builder: (_) => AddMedicineScreen(user: user)));
      ...
      Navigator.push(
          context, MaterialPageRoute(builder: (_) => SuperViewUsersScreen(user: user,)));
      ...
      Navigator.push(context,
          MaterialPageRoute(builder: (_) => finalAddMedicineStore(user: user)));
...
// import 'pages/reverse_transaction_page.dart';
    ...
    else if (option == "Reverse Debit Transaction") {
      Navigator.push(
        context,
        MaterialPageRoute(
    ...
    }else if (option == "Reverse cash Transation") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>ReverseDirectSales(userRole: '', userName: '',)),
      );
    }else if (option == "Reverse cash Transation") {
      Navigator.push(
        context,
    ...
    }else if (option == "New services") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>OtherServicesScreenAdd(user: {},)),
      );
    }else if (option == "Sales Services") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => SaleServicesScreen(user: {},)),
      );
    }else if (option == "View sales report") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => OtherSevrcesReport(userRole: '', userName: '',)),
      );
    }else if (option == "View all services") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => ServiceInventoryPage()),
      );
    }else if (option == "View Stock services History") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => ServiceLogsPage()),
      );
    }else if (option == "Scan view product") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>FullScannerPage()),
      );
    }else if (option == "Scan To sales") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>UniversalTerminalPage (user: {},)),
      );
    }else if (option == "print product qrcode") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>ViewAllQRCodesPage(user: {},)),
      );
    }else if (option == "Generate Control number") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>UniversalControlNumberPage (user: {},)),
      );
    }else if (option == "Track payment") {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) =>TrackControlNumbersPage ()),
      );
  ...
  void _openServiceMenu(String module) {
    ...
    switch (module.toUpperCase()) {
      ...
      case "SCAN TO SALES":
        opts = ["Scan To sales"];
        break;
      case "SCAN TO VIEW PRODUCT":
        opts = ["Scan view product"];
        break;
      case "DEBIT":
      ...
      case "REVERSE TRANSACTION":
        opts = ['Reverse cash Transation','Reverse Debit Transaction'];
        break;
      ...
      case "SUBSCRIPTION":
        ...
        case "ONLINE PAYMENT":
      opts = ["Generate Control number","Track payment",'Oline account','Check out'];
      break;

      case "REVERSE TRANSACTION":
        opts = ["Reverse cash Transation", "Track payment",'Oline account','Check out'];
        break;
      case "SERVICES":
        opts = ["New services", "View services",'view services logs','Track services'];
        break;
      case "SALES SERVICES":
        opts = ["Sales Services", "Reprint",];

        break;
      case "REPORT SERVICES":
        opts = ["View sales report","View all services","View Stock services History"];

        break;
      case "QRCODE PRODUCT":
        opts = ["print product qrcode"];
  ...
  Widget _buildNMBFloatingDock(Color nmbBlue) => Container(
    height: 100, // Urefu wa juu kuruhusu kitufe kielewe
    color: Colors.transparent,
    child: Stack(
      alignment: Alignment.topCenter,
      children: [
        // 1. FOOTER BACKGROUND (Rangi ya Bluu kama ulivyotaka)
        Positioned(
          bottom: 20, // Inakaa na margin ya 20 kama kodi yako ya awali
          left: 20,
          right: 20,
          child: Container(
            height: 60,
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  // Upande wa Kushoto: Home na Chat
                  Row(
                    children: [
                  ...
                  const SizedBox(width: 40), // Nafasi ya kitufe cha kati (SCAN)

                  // Upande wa Kulia: Inventory/History na Profile
                  Row(
                    children: [
                      IconButton(
        ...
        // 2. KITUFE CHA KUSCAN (Kimeelea katikati)
        Positioned(
          top: 5, // Inakifanya kielewe juu ya bar ya bluu
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(
                color: Colors.white, // Border nyeupe ya kitufe
              ...
              child: Container(
                ...
                height: 65,
                decoration: BoxDecoration(
                  color: nmbBlue, // Rangi ya Bluu ndani ya kitufe
                  shape: BoxShape.circle,
                  gradient: LinearGradient(
                    colors: [nmbBlue, const Color(0xFF003D6B)], // Bluu ya gradient
