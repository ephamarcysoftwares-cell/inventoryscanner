...
class _ReceiptScreenState extends State<ReceiptScreen> {
  ...
  Future<pw.Document> _generateThermalLayout() async {
    final pdf = pw.Document();
    final currencyFormatter = NumberFormat('#,##0', 'en_US');

    // Font sizes ndogo kwa ajili ya thermal roller (58mm)
    const double tinyFontSize = 5.0;
    const double bodyFontSize = 6.0;
    const double headerFontSize = 7.5;
    const PdfColor themeColor = PdfColor.fromInt(0xFF008080); // Teal color kama kwenye picha
    ...
    pdf.addPage(
      pw.Page(
        pageFormat: const PdfPageFormat(164, double.infinity, marginAll: 4),
        build: (pw.Context context) {
          return pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              // --- HEADER (Logo upande wa kushoto, Text upande wa kulia) ---
              pw.Row(
                ...
                children: [
                  ...
                  pw.Column(
                    ...
                    children: [
                      pw.Text("RECEIPT", style: pw.TextStyle(fontSize: headerFontSize, fontWeight: pw.FontWeight.bold, color: themeColor)),
                      pw.Text(business_name.toUpperCase(), style: pw.TextStyle(fontSize: 6, fontWeight: pw.FontWeight.bold)),
                      pw.Text(businessPhone, style: pw.TextStyle(fontSize: 5)),
                    ],
                  ),
                ],
              ),
              ...
              // --- INFO TABLE (Mpangilio kama kwenye picha) ---
              pw.Table(
                border: pw.TableBorder.all(color: PdfColors.grey300, width: 0.3),
                children: [
                  _buildInfoRow("RECEIPT NO:", widget.receiptNumber, tinyFontSize),
                  _buildInfoRow("CUSTOMER:", widget.customerName.toUpperCase(), tinyFontSize),
                  _buildInfoRow("PAYMENT:", widget.paymentMethod.toUpperCase(), tinyFontSize),
                  _buildInfoRow("DATE:", widget.confirmedTime, tinyFontSize),
                ],
              ),

              pw.SizedBox(height: 4),
              pw.Text("ITEM DETAILS", style: pw.TextStyle(fontSize: 6, fontWeight: pw.FontWeight.bold, color: themeColor)),
              pw.SizedBox(height: 2),
              ...
              pw.Table(
                ...
                children: [
                  // Table Header
                  pw.TableRow(
                    decoration: const pw.BoxDecoration(color: themeColor),
                    children: [
                      _buildCell("Description", 5, PdfColors.white, true),
                      _buildCell("Qty", 5, PdfColors.white, true),
                      _buildCell("Total", 5, PdfColors.white, true),
                    ],
                  ),
                  // Table Data
                  ...List.generate(widget.medicineNames.length, (i) {
                    double subTotal = widget.medicineQuantities[i] * widget.medicinePrices[i];
                    return pw.TableRow(
                      children: [
                        _buildCell(widget.medicineNames[i], 5.5, PdfColors.black, false),
                        _buildCell("${widget.medicineQuantities[i]}", 5.5, PdfColors.black, false),
                        _buildCell(currencyFormatter.format(subTotal), 5.5, PdfColors.black, false),
                      ],
                    );
                  }),
                ],
              ...
              pw.Align(
                ...
                child: pw.Container(
                  ...
                  decoration: pw.BoxDecoration(border: pw.Border.all(color: themeColor, width: 0.8)),
                  child: pw.Text(
                    "TOTAL PAID: TSH ${currencyFormatter.format(widget.totalPrice)}",
                    style: pw.TextStyle(fontSize: 6.5, fontWeight: pw.FontWeight.bold, color: themeColor),
                  ),
                ),
              ),
              ...
              pw.Center(
                child: pw.Column(
                  children: [
                    ...
                    pw.Text(widget.receiptNumber, style: pw.TextStyle(fontSize: 4)),
                  ],
                ),
              ),
              ...
              // --- STAMPU (BLUE STAMP) ---
              pw.Center(child: _buildSmallBlueStamp(themeColor)),

              pw.SizedBox(height: 2),
              pw.Center(
                child: pw.Text("AUTHORIZED BY: ${widget.confirmedBy.toUpperCase()}",
...
// Helper ya kutengeneza mistari ya Info
  pw.TableRow _buildInfoRow(String label, String value, double size) {
    return pw.TableRow(children: [
      pw.Padding(padding: const pw.EdgeInsets.all(2), child: pw.Text(label, style: pw.TextStyle(fontSize: size, fontWeight: pw.FontWeight.bold))),
      pw.Padding(padding: const pw.EdgeInsets.all(2), child: pw.Text(value, style: pw.TextStyle(fontSize: size))),
    ]);
  }

// Helper ya kutengeneza Cell za table
  pw.Widget _buildCell(String text, double size, PdfColor color, bool isHeader) {
    return pw.Padding(
...
// Helper ya Stempu ya Bluu
  pw.Widget _buildSmallBlueStamp(PdfColor themeColor) {
    return pw.Container(
      width: 80,
      padding: const pw.EdgeInsets.all(2),
      ...
      child: pw.Column(
        children: [
          pw.Text(business_name.toUpperCase(), style: pw.TextStyle(fontSize: 4.5, color: PdfColors.blue, fontWeight: pw.FontWeight.bold), textAlign: pw.TextAlign.center),
          pw.Text("OFFICIAL STAMP", style: pw.TextStyle(fontSize: 3.5, color: PdfColors.red)),
          pw.Divider(height: 2, thickness: 0.2, color: PdfColors.blue),
          pw.Text("DATE: ${DateFormat('dd MMM yyyy').format(DateTime.now())}", style: pw.TextStyle(fontSize: 4, color: PdfColors.blue)),
        ],
      ),
    );
  }

// Function ya kutengeneza Muhuri Mdogo (Thermal Stamp)
  pw.Widget _buildSmallThermalSeal() {
    return pw.Container(
      width: 100, // Upana mdogo ulingane na roller
      height: 65,
      padding: const pw.EdgeInsets.all(2),
      decoration: pw.BoxDecoration(
      ...
      child: pw.Column(
        ...
        children: [
          pw.Text("OFFICIAL STAMP", style: pw.TextStyle(fontSize: 5, fontWeight: pw.FontWeight.bold, color: PdfColors.red)),
          pw.Divider(height: 4, thickness: 0.5, color: PdfColors.blue),
          pw.Text(business_name.toUpperCase(), textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 6, fontWeight: pw.FontWeight.bold)),
          pw.Text(DateFormat('dd MMM yyyy').format(DateTime.now()), style: pw.TextStyle(fontSize: 6, color: PdfColors.blue)),
          pw.Text("PAID", style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: PdfColors.green)),
        ],
      ),
    );
  }
              ...
              children: [
                Expanded(
                  child: _actionButton(
                    ...
                    color: Colors.blue,
                    onTap: () => _handleReceiptAction("view"),
                  ),
                ),
                ...
                Expanded(
                  child: _actionButton(
                    ...
                    color: Colors.green,
                    onTap: () async {
                      // Inatumia ile ile function yako uliyotengeneza yenye Logo na Mhuri
                      final pdf = await _generateReceiptPdf();

                      // Inatuma kwenye printer ikiwa imejirekebisha (scaled) kutoshea 58mm
                      await Printing.layoutPdf(
                        onLayout: (PdfPageFormat format) async => pdf.save(),
                        name: 'Receipt_${widget.receiptNumber}',
                        // Hii roll57 ndiyo inayofanya PDF ya A4 isagike kutoshea printer ndogo
                        // Pia inachochea printer ikate karatasi (Auto-cut) mwishoni
                        format: PdfPageFormat.roll57,
                        usePrinterSettings: true, // Inatumia vipimo vya printer yako husika
                      );
                    },
                  ),
                ),
