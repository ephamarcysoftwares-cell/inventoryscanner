...
// NOTE: The 'sendWhatsApp' function must be defined and accessible for this code to compile.
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (userData == null) {
        _showError('Profile haikupatikana');
        return;
      }
      ...
      // 3. GRACE PERIOD: SIKU 5 ZA LAST SEEN
      ...
      // 4. VALIDATE PAYMENT IF GRACE PERIOD EXPIRED
      if (!yupoNdaniYaSiku5) {
        ...
        if (paymentData == null) {
          _showBlockingDialog("JARIBIO LIMEISHA", "Siku 5 zimeisha na hakuna malipo yaliyopatikana.", businessName, businessId);
          return;
        }

        // --- CHEKI KIASI KILICHOLIPWA (AMOUNT VS COLLECTED AMOUNT) ---
        ...
        if (collected < amountDue) {
          _showBlockingDialog(
              "MALIPO HAYAJATOSHA",
              "Malipo yako ya TZS ${paymentData['collected_amount']} hayajatosheleza kiasi cha TZS ${paymentData['amount']}. Tafadhali kamilisha malipo.",
              businessName,
              businessId
          );
          return;
        ...
        // --- CHEKI TAREHE YA KUISHA ---
        DateTime expiryDate = DateTime.parse(paymentData['expiry_date']);
        if (leo.isAfter(expiryDate)) {
          _showBlockingDialog(
              "HUDUMA IMEISHA",
              "Muda wako umeisha tarehe ${DateFormat('dd/MM/yyyy').format(expiryDate)}.",
              businessName,
              businessId
          );
          return;
      ...
      // 5. SUCCESS: UPDATE LAST SEEN AND PROCEED
