...
class SubBranchRegisterScreenSfaff extends StatefulWidget {
  const SubBranchRegisterScreenSfaff({super.key});

  @override
  _SubBranchRegisterScreenSfaffState createState() => _SubBranchRegisterScreenSfaffState();
}

class _SubBranchRegisterScreenSfaffState extends State<SubBranchRegisterScreenSfaff> {
  ...
  // Orodha ya Role zote na majina yake ya kuonyesha (Display Labels)
  final List<Map<String, String>> _roleOptions = [
    ...
    {'value': 'Secretary', 'label': 'Secretary (Stationary)'},
    {'value': 'it', 'label': 'IT Support'},
    {'value': 'hr', 'label': 'HR (Rasilimali Watu)'},
    {'value': 'Receptionist', 'label': 'Receptionist (Mapokezi)'},
    {'value': 'storekeeper', 'label': 'Storekeeper (Mtunza Stoo)'},
  ];
    ...
    try {
      ...
      if (currentUserRole == 'admin') {
        ...
        setState(() {
          ...
          _selectedRole = 'staff'; // Default role
        });
      }
    } catch (e) {
  ...
  // Hii inachuja role ambazo mtumiaji anaruhusiwa kusajili
  List<Map<String, String>> _getFilteredRoles() {
    if (currentUserRole == 'admin') {
      return _roleOptions; // Admin anaona zote
    } else {
      // Sub-Admin hawezi kusajili Sub-Admin mwenzake
      return _roleOptions.where((r) => r['value'] != 'sub_admin').toList();
    }
  }
  ...
  Future<void> handleRegistration() async {
    ...
    try {
      ...
      if (e.message.contains('already registered') || e.statusCode == '422') {
        ...
        setState(() => errorMessage = "Hitilafu ya usajili: ${e.message}");
      }
    } catch (e) {
                ...
                children: [
                  ...
                  _buildDropdown<Map<String, dynamic>>(
                    ...
                    onChanged: currentUserRole == 'admin'
                        ...
                        : null, // Sub-admin hawezi kubadili tawi lake mwenyewe
                  ),
                ],
              ),
            ),
  ...
  Widget _buildDropdown<T>({
    ...
    // ✅ 1. Uhakiki: Angalia kama value ipo ndani ya items zilizotolewa
    // Hii inazuia crash kama value ni 'staff' lakini 'staff' haipo kwenye items
    final bool valueExists = items.any((item) => item.value == value);

    return DropdownButtonFormField<T>(
      // ✅ 2. Kama value haipo kwenye list, weka null ili dropdown isicrash
      value: valueExists ? value : null,
      items: items,
      isExpanded: true,
      onChanged: onChanged,
      // Ongeza validation kuzuia tupu
      validator: (val) => val == null ? 'Field hii ni lazima' : null,
  ...
  void _showSuccess(String name, String role) {
    // Tafuta jina la role kwa ajili ya kuonyesha kwenye alert
    String roleLabel = _roleOptions.firstWhere((r) => r['value'] == role)['label']!;
    showDialog(
      context: context,
