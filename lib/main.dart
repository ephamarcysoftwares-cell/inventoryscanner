...
import 'package:stock_and_inventory_software/splash_screen.dart';
...
void main() async {
  ...
  // 1. Anzisha Supabase (Hii haina shida na plugin implementation)
  await Supabase.initialize(
    url: 'https://tfqbfzllkducffjoeqsi.supabase.co',
    anonKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRmcWJmemxsa2R1Y2Zmam9lcXNpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjU5MDcxMDUsImV4cCI6MjA4MTQ4MzEwNX0.oH-Wh-6rfgpcYjX-1jw2j4s9jrvCbVIJ9_iBg7hW1D0',
  );

  // 2. Anzisha App haraka iwezekanavyo
  runApp(const MyApp());

  // 3. Shughulikia Windows Window baada ya App kuanza (Post-frame callback)
  if (Platform.isWindows) {
    // Tunatumia try-catch kuzuia kosa la MissingPluginException lisiue App
    try {
      ...
      WindowOptions windowOptions = const WindowOptions(
        size: Size(1280, 800),
        center: true,
        title: "Arusha Online Store - Universal Terminal",
      );
      ...
      // Kama implementation haijapatikana, App itaendelea kufanya kazi kwenye window ya kawaida
      debugPrint("WINDOW_MANAGER_ERROR: $e");
    }
  }

  Future.delayed(const Duration(seconds: 5), () {
    try { checkForUpdates(); } catch (e) { debugPrint("Update error: $e"); }
  });
}
